...
export interface InterviewRoomSessionProps {
  id: string;
  listPath: string;
  roomPath: string;
  resultPath: string;
  loadInterview: (id: string) => Promise<InterviewSessionView>;
  startInterview?: (id: string) => Promise<InterviewSessionView>;
  answerQuestion: (
    interviewId: string,
    questionItemId: string,
    formData: FormData,
  ) => Promise<InterviewAnswerUpload>;
  submitInterview: (id: string) => Promise<unknown>;
}

export default function InterviewRoomSession({
  id,
  listPath,
  roomPath,
  resultPath,
  ...
  // Feature quyết định có cần bước start riêng hay không qua startInterview.
  ...
  }, [hydrated, user, id, router, loadMock, roomPath]);
